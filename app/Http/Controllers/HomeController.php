...
class HomeController extends Controller
{
    ...
    public function index()
    {
        ...
        $random_category_products = Cache::remember('random_category_products', 3600, function () {
            return Category::with(['products' => function ($query) {
                $query->inRandomOrder(); // Shuffle products within each category
            }])->has('products')->get()->flatMap->products->shuffle()->take(100); // Flatten, shuffle, and limit to 100
        });



        return view('frontend.index', compact('featured_categories', 'todays_deal_products', 'newest_products', 'random_category_products'));
    }
    ...
    public function updateSession(Request $request)
    {
        ...
        if (auth()->user() != null) {
            ...
            Cart::where('user_id', $user_id)->update([
                'shipping_cost' => $value / count($carts),
            ]);
        } else {
