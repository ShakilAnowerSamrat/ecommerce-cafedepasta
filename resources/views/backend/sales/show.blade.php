                ...
                <div class="col-md-4 ml-auto">
                    <table>
                        <tbody>
                            ...
                            @php
                                $piprapayPayment = null;
                                if ($order->payment_type == 'piprapay') {
                                    $piprapayPayment = \App\Models\PiprapayPayment::where('order_id', $order->id)
                                        ->orWhere('combined_order_id', $order->combined_order_id)
                                        ->latest()
                                        ->first();
                                }
                            @endphp
                            @if ($piprapayPayment)
                            <tr>
                                <td class="text-main text-bold">{{ translate('PipraPay Ref (pp_id)') }}</td>
                                <td class="text-right"><span class="badge badge-inline badge-info">{{ $piprapayPayment->pp_id }}</span></td>
                            </tr>
                            @if ($piprapayPayment->provider_transaction_id)
                            <tr>
                                <td class="text-main text-bold">{{ translate('Gateway Trx ID') }}</td>
                                <td class="text-right">{{ $piprapayPayment->provider_transaction_id }}</td>
                            </tr>
                            @endif
                            @if ($piprapayPayment->fee > 0)
                            <tr>
                                <td class="text-main text-bold">{{ translate('Gateway Fee') }}</td>
                                <td class="text-right">{{ single_price($piprapayPayment->fee) }}</td>
                            </tr>
                            @endif
                            @if ($piprapayPayment->isRefunded())
                            <tr>
                                <td class="text-main text-bold">{{ translate('Refund Status') }}</td>
                                <td class="text-right"><span class="badge badge-inline badge-danger">{{ translate('Refunded') }}</span></td>
                            </tr>
                            @elseif ($order->payment_status == 'paid')
                            <tr>
                                <td class="text-main text-bold">{{ translate('Gateway Actions') }}</td>
                                <td class="text-right">
                                    <form action="{{ route('admin.piprapay.refund') }}" method="POST" onsubmit="return confirm('{{ translate('Are you sure you want to refund this order via PipraPay?') }}')">
                                        @csrf
                                        <input type="hidden" name="order_id" value="{{ $order->id }}">
                                        <button type="submit" class="btn btn-xs btn-danger">{{ translate('Refund via PipraPay') }}</button>
                                    </form>
                                </td>
                            </tr>
                            @endif
